"""
experiments/run_ssm.py
───────────────────────
S4-lite (SSM) baseline experiment runner.

Usage
-----
    python experiments/run_ssm.py
...
def run_ssm(cfg) -> dict:
    """Full SSM baseline training + evaluation."""
    from models.ssm import SSMModel
    ...
    log.info("=== SSM (S4-lite) Baseline Experiment ===")
    ...
    model = SSMModel(cfg)
    log.info(f"SSM params: {model.count_parameters():,}")
    ...
    log.info(f"SSM | AUROC={ev.get('auroc',0):.4f} | AUPRC={ev.get('auprc',0):.4f} | F1={ev.get('macro_f1',0):.4f}")
    return {"history": history, "evaluation": ev, "full_report": full_report, "out_dir": str(out_dir)}


def _parse_args():
    parser = argparse.ArgumentParser(description="SSM baseline runner")
...
def main():
    ...
    cfg = load_config(args.config, overrides=["model.name=ssm"] + (overrides or []))
    setup_logger(cfg.experiment.log_dir, cfg.experiment.name + "_ssm",
