"""
experiments/run_transformer.py
────────────────────────────────
Causal Transformer baseline experiment runner.

Usage
-----
    python experiments/run_transformer.py
...
def run_transformer(cfg) -> dict:
    """Full Transformer baseline training + evaluation."""
    ...
    log.info("=== Transformer Baseline Experiment ===")
    ...
    log.info(f"Transformer params: {model.count_parameters():,}")
    ...
    log.info(f"Transformer | AUROC={ev.get('auroc',0):.4f} | AUPRC={ev.get('auprc',0):.4f} | F1={ev.get('macro_f1',0):.4f}")
...
def main():
    ...
    cfg = load_config(args.config, overrides=["model.name=transformer"] + (overrides or []))
    setup_logger(cfg.experiment.log_dir, cfg.experiment.name + "_transformer",
